	...
	async execute(message) {
		...
		if (result_of_checks.length > 0) {
			const guildChannels = await message.guild.channels.fetch(),
				guildRoles = await message.guild.roles.fetch(),
			...
			if (bladeChannels.includes(postChannel)) { // if the post was made in a blade channel, returns true
				...
				if (outputStyle == "Legacy") {
					guildChannels.find(channel => channel.name === "returning-blade-alerts").send("<@&" + guildRoles.find(role => role.name === "Knights").id + ">, <@&" + guildRoles.find(role => role.name === "Squires").id + ">" + legacyOutput(output, result_of_checks, message));
				} else if (outputStyle == "Embed") {
					const toSend = await embedOutput(output, result_of_checks, message);
					guildChannels.find(channel => channel.name === "returning-blade-alerts").send({ content: "<@&" + guildRoles.find(role => role.name === "Knights").id + ">, <@&" + guildRoles.find(role => role.name === "Squires").id + ">", embeds: [toSend] });
				} else {
